...
const majorLeagues = [39, 140, 61, 78, 135]; // Liga besar
...
async function fetchLiveMatches() {
    ...
    try {
        ...
        if(matches.length === 0){
            liveDiv.innerHTML = "Tidak ada pertandingan live saat ini atau API key tidak memberi akses live.";
            return;
        }
...
async function fetchUpcomingMatches(daysAhead = 7) {
    ...
    try {
        ...
        if(matches.length === 0){
            scheduleDiv.innerHTML = "Tidak ada jadwal pertandingan dalam periode ini atau API key tidak memberi akses.";
            return;
        }
